...
struct CustomMacroProgressWeek: View {
    @Environment(MealViewModel.self) var mealVM
    
    var selectedDay: Date
    
    var body: some View {
        let filtermeal = mealVM.mealList.filter { $0.date.isInSameWeek(as: selectedDay) }
